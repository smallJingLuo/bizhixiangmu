...
- (void) calculateExpandedCoordinates{
    ...
    _mainButton.frame = CGRectMake((width -_mainButtonImage.size.width)/2, height / 2 + 100 - _mainButtonImage.size.height / 2, _mainButtonImage.size.width, _mainButtonImage.size.height);
    NSArray *titleArray = @[@"综合",@"分类",@"搜索"];
    for (int i = 0;i<_choices.count;i++){
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10,buttonChoiceImage.size.height - 20, buttonChoiceImage.size.width - 20, 20)];
        label.text = titleArray[i];
        label.textAlignment = NSTextAlignmentCenter;
        label.backgroundColor = [UIColor colorWithRed:240.0/255.0 green:245.0/255.0 blue:250.0/255.0 alpha:1];
